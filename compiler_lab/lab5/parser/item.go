...
func (it Item) String(g *syntax.Grammar) string {
	prod := g.Productions[it.ProdIndex]
	right := make([]syntax.Symbol, len(prod.Right)+1)
...
// Jion help print
func jion(syms []syntax.Symbol) string {
	str := ""
	for _, s := range syms {
		str += string(s) + " "
	}
...
// Closure computes the closure of a set of items in the context of a syntax.
func Closure(g *syntax.Grammar, items []Item) []Item {
...
// Goto computes the goto set for a given set of items and a symbol in the context of a syntax.

func Goto(g *syntax.Grammar, items []Item, symb syntax.Symbol) []Item {
	var moved []Item
