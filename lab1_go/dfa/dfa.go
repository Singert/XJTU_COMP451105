...
func (d *DFA) CheckValidity() bool {
	...
	//6️⃣ check transitions in states
	fmt.Print("checking transitions in states:")
	for state, trans := range d.Transitions {
		if !contains(d.States, state) {
			fmt.Printf("❌ TranState %s not in states\n", state)
			fmt.Println("[DFA Invalid]")
			return false
		}
		for symbol, dest := range trans {
			if !contains(d.Alphabet, symbol) {
				fmt.Printf("❌ TranSymbol %s not in alphabet\n", symbol)
				fmt.Println("[DFA Invalid]")
				return false
			}
			if !contains(d.States, dest) {
				fmt.Printf("❌ TranDest %s not in states\n", dest)
				fmt.Println("[DFA Invalid]")
				return false
			}
		}
	...
	fmt.Print("PASS\n")
	//7️⃣ check every state has a transition for every symbol
	fmt.Print("checking every state has a transition for every symbol:")
	for _, state := range d.States {
		transitions, exists := d.Transitions[state]
		if !exists {
			fmt.Printf("❌ State %s has no transitions\n", state)
			fmt.Println("[DFA Invalid]")
			return false
		}

		for _, symbol := range d.Alphabet {
			_, exists := transitions[symbol]
			if !exists {
				fmt.Printf("❌ State %s has no transition for symbol %s\n", state, symbol)
				fmt.Println("[DFA Invalid]")
				return false
			}
		}
...
func (d *DFA) ExportToDot(filename string, trace []TransitionTrace) error{
	...
	for from, trans := range d.Transitions {
		for symbol, to := range trans {
			...
			if edgeMark[key] {
				fmt.Fprintf(file, `  %s -> %s [label=%s, color=red, penwidth=2];`+"\n", from, to, symbol)
			} else {
				fmt.Fprintf(file, `  %s -> %s [label=%s];`+"\n", from, to, symbol)
			}
		}
